...
if(isset($_REQUEST['product_id'])){
    ...
    global $con;
    $statement = $con->prepare("UPDATE products SET status = ? where id = ?");
    if (!$statement)
        echo "Statement could not be created";
    // See parameters binding .. before initialization ... binding is done/allowed
    $statement->bind_param("ii", $status, $prod_id);
    $statement->execute() or die("Could not execute DB statement.");
    header("Location:http://localhost/webtech/Ecommerce/dashboard-admin.php?message=Product approved successfully", true, 302);
}
?>
</body>
</html>